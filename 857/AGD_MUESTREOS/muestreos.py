...
with monitoreo.connect() as connection2:
    ...
    query2 = """SELECT ID_MUESTREO, ID_ESTACION, ID_PROYECTO, FECHA, OBSERVACIONES FROM VM_DATOS_MONITOREO WHERE ID_PROYECTO = 2427 AND ID_TEMATICA = 71 """
    ...
    for _, df_muestra in datos2Df.groupby('ID_MUESTREO'):
        ...
        insertMuestreo = f"""INSERT INTO AGD_MUESTREOS (ID_MUESTREO, ID_ESTACION, ID_PROYECTO, ID_METODOLOGIA, ID_TEMATICAS, FECHA)
                        VALUES({str(857)+str(df_muestra['ID_MUESTREO'].values[0])},{df_muestra['ID_ESTACION'].values[0]}, {3492}, {857}, {217}, TO_DATE('{str(df_muestra['FECHA'].values[0]).replace('T',' ').split('.')[0]}','YYYY-MM-DD HH24:MI:SS'))"""
