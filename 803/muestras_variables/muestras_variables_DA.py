...
with engine.connect() as connection2:
    ...
    query2 = """SELECT CASE
                ...
                WHERE VARIABLE IS NULL AND ID_MUESTREO IN (81620180723105032) AND UNIDADES IN (67) AND FECHA_HORA LIKE ('%/2018%')"""
    ...
    for _, df_muestra in datos2Df.groupby('ID_MUESTRA'):
        ...
        insertDA = f"""INSERT INTO AGD_MUESTRAS_VARIABLES (ID_PARAMETRO, ID_METODOLOGIA, ID_UNIDAD_MEDIDA, ID_MUESTRA, ID_METODO, VALOR) 
        VALUES({143},{803},{67},{df_muestra['ID_MUESTRA'].values[0]}, {667}, {df_muestra['VALOR'].values[0]})"""
        
        muestras.append(insertDA)
    ...
    muestras.to_csv('muestras_variables_DA.csv', index=False)
