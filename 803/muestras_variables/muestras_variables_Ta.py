...
with engine.connect() as connection2:
    ...
    query2 = """SELECT CASE
                ...
                WHERE VARIABLE IN ('484') AND ID_MUESTREO IN (81620180118151515,81620180119151148,81620180122132412,
                81620180122160828,81620180411161602,81620180914081944,81620181130080509,81620181228134507,81620190514144057,81620190813084908)"""
    ...
    for _, df_muestra in datos2Df.groupby('ID_MUESTRA'):
        ...
        insertTa = f"""INSERT INTO AGD_MUESTRAS_VARIABLES (ID_PARAMETRO, ID_METODOLOGIA, ID_UNIDAD_MEDIDA, ID_MUESTRA, ID_METODO, VALOR)
                        VALUES({484},{803},{5},{df_muestra['ID_MUESTRA'].values[0]}, {685}, {df_muestra['VALOR'].values[0]})"""
        
        muestras.append(insertTa)
    ...
    muestras.to_csv('muestras_variables_Ta.csv', index=False)
