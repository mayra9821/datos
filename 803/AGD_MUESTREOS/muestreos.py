...
with engine.connect() as connection2:
    ...
    query2 = """SELECT ID_MUESTREO, ID_ESTACION, ID_PROYECTO, FECHA, NOTAS_GENERALES
                ...
                WHERE VARIABLE IN ('140','141','142','143','483','484','482') and PROYECTO_ORIGEN='INAP' """
    ...
    for _, df_muestra in datos2Df.groupby('ID_MUESTREO'):
        ...
        VALUES({str(2069)+str(df_muestra['ID_MUESTREO'].values[0])},{df_muestra['ID_ESTACION'].values[0]}, {2507}, {803}, {213}, TO_DATE('{str(df_muestra['FECHA'].values[0]).replace('T',' ').split('.')[0]}','YYYY-MM-DD HH24:MI:SS'))"""
