...
with monitoreo.connect() as connection2:
    ...
    query2 = """SELECT ID_MUESTREO, ID_CUALIDAD, VALOR_NUM FROM VM_DATOS_MONITOREO WHERE ID_PROYECTO = 1480 
                AND ID_MUESTREO IN(3220201203291317121, 3226201308060747281, 3220201203291046101)"""
    ...
    datos2Df['COMPLEMENTO'] = datos2Df['ID_CUALIDAD'].apply(lambda x: x.strip().replace(" ","")[3:])
    ...
    for _, df_muestra in datos2Df.groupby('ID_MUESTRA'):
        ...
        insertLuminosidad = f"""INSERT INTO AGD_MUESTRAS_VARIABLES (ID_PARAMETRO, ID_METODOLOGIA, ID_UNIDAD_MEDIDA, ID_MUESTRA, ID_METODO, VALOR)
                        VALUES({434},{857},{53},{df_muestra['ID_MUESTRA'].values[0]}, {624}, {df_muestra['VALOR_NUM'].values[0]})"""
        
        muestras.append(insertLuminosidad)
    ...
    muestras.to_csv('muestras_variables_luminosidad.csv', index=False)
