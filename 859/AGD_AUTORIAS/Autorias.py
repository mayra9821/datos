...
with monitoreo.connect() as connection2:
    ...
    query2 = """SELECT ID_MUESTREO, FECHA_HORA, ID_ENTIDAD FROM BMUESTREOS WHERE ID_PROYECTO=3023  
    ...
    for _, df_muestra in datos2Df.groupby('ID_MUESTREO'):
        ...
        VALUES({4835},{5},{1},TO_DATE('{str(df_muestra['FECHA_HORA'].values[0]).replace('T',' ').split('.')[0]}', 'YYYY-MM-DD HH24:MI:SS'),{df_muestra['ID_MUESTREO'].values[0]}, '{df_muestra['ID_ENTIDAD'].values[0]}')"""
        
        insertAutor2 = f"""INSERT INTO AGD_AUTORIAS (ID_FUNCIONARIO, ID_TAREA, ORDEN, FECHA, ID_MUESTRA, ENTIDAD) 
        VALUES({5845},{4},{2},TO_DATE('{str(df_muestra['FECHA_HORA'].values[0]).replace('T',' ').split('.')[0]}', 'YYYY-MM-DD HH24:MI:SS'),{df_muestra['ID_MUESTREO'].values[0]}, '{df_muestra['ID_ENTIDAD'].values[0]}')"""
