...
# to_number(substr(to_char((to_number(to_char(to_date(substr(id_cualidad, 1, 8), 'YYYYMMDD'), 'J')) - 2415019 ) + to_number(substr(id_cualidad, 10, 2)) / 24 +
#                 to_number(substr(id_cualidad, 12, 2)) / ( 60 * 24 ) + to_number(substr(id_cualidad, 14, 5)) / ( 3600 * 24 )),1,21)) AS FECHA
    query2 = """SELECT CASE
                    ...
                    vm_agm_2507_816
                    WHERE
                    variable IN ( '151','12','52','56','89','826')
                    AND id_muestreo IN (81620190510072821)"""
    ...
    for _, df_muestra in datos2Df.groupby('ID_MUESTRA'):
        ...
        insertFecha = f"""INSERT INTO AGD_MUESTRAS_VARIABLES (ID_PARAMETRO, ID_METODOLOGIA, ID_UNIDAD_MEDIDA, ID_MUESTRA, VALOR)
                        VALUES({646},{858},{100},{df_muestra['ID_MUESTRA'].values[0]}, '{df_muestra['FECHA_ARREGLADA'].values[0]}')"""
