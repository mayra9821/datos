...
with engine.connect() as connection2:
    query2 = """SELECT CASE
                ...
                WHERE VARIABLE IN ('151','12','52','56','89','826') AND ID_MUESTREO IN (81620190510072821)
                """
    ...
    for _, df_muestra in datos2Df.groupby('ID_MUESTRA'):
        ...
        insertMuestra = f"""INSERT INTO AGD_MUESTRAS (ID_MUESTRA, ID_MUESTREO, ES_REPLICA)
                        VALUES({df_muestra['ID_MUESTRA'].values[0]},{str(858)+str(df_muestra['ID_MUESTREO'].values[0])},{df_muestra['ES_REPLICA'].values[0]})"""
