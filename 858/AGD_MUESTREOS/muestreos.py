...
with engine.connect() as connection2:
    ...
    query2 = """SELECT ID_MUESTREO, ID_ESTACION, ID_PROYECTO, FECHA
                ...
                WHERE VARIABLE IN ('151','12','52','56','89','826') AND PROYECTO LIKE 'GIRCC'"""
    ...
    for _, df_muestra in datos2Df.groupby('ID_MUESTREO'):
        ...
        insertMuestreo = f"""INSERT INTO AGD_MUESTREOS (ID_MUESTREO, ID_ESTACION, ID_PROYECTO, ID_METODOLOGIA, ID_TEMATICAS, FECHA)
                        VALUES({str(858)+str(df_muestra['ID_MUESTREO'].values[0])},{df_muestra['ID_ESTACION'].values[0]}, {2507}, {858}, {217}, TO_DATE('{str(df_muestra['FECHA'].values[0]).replace('T',' ').split('.')[0]}','YYYY-MM-DD HH24:MI:SS'))"""
